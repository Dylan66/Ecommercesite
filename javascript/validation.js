...
function validate_store_change(form, type){
    ...
    if(ret){        
       ...
       ajax("backend/Tenant.php","manageStore=set&"+data+"&new="+val,"post","backend/Handler.php","store=true&"+getData,"get","ajax_store");
    }
...
function confirm_delete(src){
    return confirm("Are you sure you want to delete the "+src);
}

function confirm_delete_store(src, id){
    var getData = "stores=set";
    var postData = "delete=set&id="+id+"&type="+src;
    
    if(confirm_delete(src)){        
        ajax("backend/Tenant.php",postData,"post","backend/Handler.php",getData,"get","ajax_stores");
    }
...
function validate_modify_location(form, loc_id){
    ...
    var postData = "modifyLocation=set&id="+loc_id+"&street="+form.street.value+"&building="+form.building.value+"&stall="+form.stall.value;
...
function validate_outlet_form(form){
    ...
    var postData = "addLocation=set&street="+form.street.value+"&building="+form.building.value+"&stall="+form.stall.value+"&id="+form.id.value;
    if(form.street.value.length < 3){
        err.innerHTML = "* Enter a valid Street name";
        ret = false;
    } else if(form.building.value.length < 3){
...
function delete_loc(id, store_id){
    var getData = "locations=set";
    var postData = "deleteLocation=set&id="+id;
    
    if(confirm_delete("outlet")){        
        ajax("backend/Tenant.php",postData,"post","backend/Handler.php",getData+"&id="+store_id,"get","ajax_outlets");
    }
...
function validate_remove_image(form, id, name){
    var getData = "";
    var postData = "removeImage=set&img_id="+id+"&name="+name;
    
    if(confirm_delete("Image")){        
        ajax("backend/Tenant.php",postData,"post","backend/Handler.php",getData,"get", "ajax_"+id);
    }
...
function validate_product_listing(form){
    ...
    var postData = "addProductServiceListing=set&new="+form.new.value+"&id="+form.id.value;
    err.innerHTML = "";
    if(form.new.value.length < 20){
        err.innerHTML = "* The description should be atleast 20 characters";
        ret = false;
    } 
    if(ret){        
       form.new.value = "";
        ajax("backend/Tenant.php",postData,"post","backend/Handler.php",getData,"get","ajax_products_listing");
    }
...
function validate_delete_product_listing(form, id){
    var getData = "productServiceListing=set&id="+form.id.value;
    var postData = "removeProductServiceListing=set&id="+id;

    if(confirm_delete("Product Listing")){        
        ajax("backend/Tenant.php",postData,"post","backend/Handler.php",getData,"get","ajax_products_listing");
    }
