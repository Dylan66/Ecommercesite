...
header('location:login.php');
	...
	if(isset($_POST['update']))
	{
		...
		$query=mysqli_query($con,"UPDATE users set name='$name',contactno='$contactno' where user_id='".$_SESSION['id']."'");
		if($query)
		{
...
{
	$me = $_SESSION['id'];
	$sql=mysqli_query($con,"SELECT password FROM  users where password='".md5($_POST['cpass'])."' AND user_id='$me'");
	$num = mysqli_fetch_array($sql);
	// print($password); echo "<br>";
	
	if($num>0)
	{
		$pass = md5($_POST['newpass']);
	 	$sql = mysqli_query($con,"UPDATE users set password='$pass', updationDate='$currentTime' WHERE user_id='$me'");
	 	if ($sql) {
	 		echo "<script>alert('Password Changed Successfully !!');window.location='my-account.php';</script>";
	 	}
	 	else
	 	{
